...
test('Thursday projector shows every activity title and keeps teacher notes out',()=>{delete globalThis.window;const {DayPlanMorning}=load('app/day-plan-library.tsx');const plan={...structuredClone(store.publishedDayPlans[0]),note:'TEACHER ONLY NOTE',reflection:'PRIVATE REFLECTION'};assert.equal(plan.date,'2026-09-24');const html=renderToStaticMarkup(React.createElement(DayPlanMorning,{plan}));assert.match(html,/Welcome to Grade 6/);assert.match(html,/Shape of our day/);for(const block of plan.blocks)assert.ok(html.includes(block.title.replaceAll('&','&amp;')),block.title);assert.doesNotMatch(html,/TEACHER ONLY NOTE|PRIVATE REFLECTION|supervising adult/);assert.ok(store.publishedDayPlans.some(p=>p.date==='2026-09-23'&&p.blocks.some(b=>b.title==='Werewolf')));});
