...
export const projectorLessonSupports: Record<string, ProjectorLessonSupport> = {
  ...
  "same-facts-frame": support("same-facts-frame", {
    purpose: "Arrange the same facts in two ways and explain how scale, colour, spacing, and focus change the feeling.",
    background: [
      "Composition is the arrangement of parts. It guides where an audience looks first and how the parts feel connected.",
      "Two designs may contain the same accurate facts while emphasizing different stories about those facts.",
      "Large type, strong contrast, repeated shapes, empty space, and sound rhythm all create emphasis.",
      "A responsible creator can name the intended effect and avoid hiding a fact the audience needs.",
    ],
    terms: terms(
      ["composition", "the way parts are arranged in an artwork", "The largest number sits in the centre with wide empty space around it."],
      ["scale", "the size of one part compared with another", "Making 42 much larger than 18 gives 42 more importance."],
      ["contrast", "a strong difference that makes something stand out", "White text on a dark purple block creates high contrast."],
      ["rhythm", "a repeated visual, sound, or movement pattern", "Forty-two small marks create a busy rhythm across the page."],
      ["framing", "choices that shape which part of a subject the audience sees", "One poster frames the library night as busy; another frames it as quiet reading."],
    ),
    example: {
      title: "Frame one fictional library night two different ways",
      steps: [
        "Shared facts: the library stayed open until 4:00 on Tuesday, 42 people visited, and 18 items were checked out.",
        "Frame A places ‘42 visits’ in huge bright type above 42 repeated doorway marks. The feeling is busy and social.",
        "Frame B places ‘18 checkouts’ alone in a quiet dark field with one book shape. The feeling is calm and focused on borrowing.",
        "Both designs include all three facts in readable text. Neither changes a number.",
        "Artist explanation: ‘I changed scale, spacing, and rhythm to shift attention from gathering to reading.’",
      ],
      conclusion: "Facts set a boundary; composition changes the audience's path through those facts.",
    },
    checks: [
      {
        prompt: "Which choice creates the strongest emphasis on 42 visits?",
        choices: ["Make every fact the same size.", "Make 42 large, high-contrast, and surrounded by repeated doorway marks.", "Remove the number 42."],
        answer: 1,
        feedback: "Size, contrast, and repetition all guide attention toward the visit count.",
      },
      {
        prompt: "When do two different frames stay responsible?",
        choices: ["When they keep the needed facts accurate and readable", "When one hides any fact that weakens the mood", "When the audience cannot tell what choices were made"],
        answer: 0,
        feedback: "A creator may shape feeling, but important facts still need to remain accurate and available.",
      },
    ],
    reteach: "Show the same three facts in identical type first. Change only scale, then only colour, so the effect of each choice is easier to see.",
  }),

  "rights-in-thirty": support("rights-in-thirty", {
    purpose: "Use a frozen scene, one change, and simple sound to show a barrier and a possible repair without acting out harm.",
    background: [
      ...
      "A tableau can show power and access through distance, level, direction, focus, and gesture without reenacting a harmful event.",
    ...
    terms: terms(
      ...
      ["remedy", "an action that repairs or reduces a problem", "Add a step-free entrance and information in more than one format."],
