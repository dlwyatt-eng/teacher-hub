...
export function MathResourceWorkbench({ experienceId, allTopics = false, projector = false }: { experienceId?: string; allTopics?: boolean; projector?: boolean }) {
  ...
  return <section className={`math-workbench ${projector?'math-workbench--projector':''}`} aria-label="Mathematics teaching resources">
    ...
    {panel==='worksheets' && <div className="math-workbench-panel"><div className="math-workbench-grid">{slugsFor(pack.id,experienceId).map(slug=>{const video=mathAnticsCatalogue[slug];return <article key={slug}><small>MATH ANTICS</small><h3>{video.title}</h3><div className="math-workbench-links"><a href={video.url} target="_blank" rel="noreferrer">Open Math Antics lesson &amp; worksheets ↗</a><a href={video.youtube} target="_blank" rel="noreferrer">Watch video ↗</a></div></article>;})}</div><p>{task?.paper}</p><p className="math-access-note">Open the Math Antics lesson page and sign in with your membership to access worksheets and answer keys.</p>{!projector && <p><a href="https://drive.google.com/file/d/1iOMu49HqdI5EqpvdFHSHKVTFL3FvCIbC/view" target="_blank" rel="noreferrer">My private worksheet &amp; answer-key archive · Google Drive ZIP ↗</a><br /><small>Sign in to the Google account that owns this archive. Other teachers use their own Math Antics membership.</small></p>}</div>}
