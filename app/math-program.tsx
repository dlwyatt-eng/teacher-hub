...
function TeacherMathPack({ pack }: { pack: MathSupportPack }) {
  ...
  return (
    <article className="math-teaching-pack">
      ...
      <div className="math-pack-logistics"><section><b>SUPPLIED ON THIS PAGE</b>{pack.supplied.map(item => <span key={item}>✓ {item}</span>)}</section><section><b>GATHER</b>{pack.gather.map(item => <span key={item}>○ {item}</span>)}</section><section><b>RESOURCE SLOTS</b><span>Explanation choice: matching Math Antics video, this Hub model, or both</span><span>MathUP check / games: {pack.mathUpTopics.join(" · ")}</span>{pack.mathUpTopics.map(topic => { const link = mathUpLinkFor(topic); return link && <a key={topic} href={link.url} target="_blank" rel="noreferrer">{topic} · {link.label} ↗</a>; })}<span>Optional: teacher-owned practice or organizer</span><em>Preview MathUP activities for fit. The on-page investigation remains usable with one projector and ordinary classroom materials.</em></section></div></>}
...
export function MathYearImplementation({ program }: { program: LearningProgram }) {
  ...
  return (
    <section className="math-year-implementation">
      ...
      <p><a className="math-routines-link" href="?view=Math+Thinking+Routines">Open math thinking routines · choose one short start →</a></p>
      <MathPacingPanel />
      <MathAnticsYearIntro />
      <MathGamesIntro />
      <div className="math-calendar-list">{mathYearSequence.map((item, index) => <article key={`${item.timing}-${item.focus}`}><span>{String(index + 1).padStart(2, "0")}</span><header><small>{item.timing} · {item.blocks}</small><h3>{item.focus}</h3><div>{item.mathUpTopics.map(topic => { const link = mathUpLinkFor(topic); return link ? <a key={topic} href={link.url} target="_blank" rel="noreferrer" title={`MathUP ${link.label} for ${topic}`}>{topic} ↗</a> : <b key={topic}>{topic}</b>; })}</div></header><section><p><strong>TEACH / APPLY:</strong> {item.lessonIds.map(nameFor).join(" → ")}</p><p><strong>FLUENCY:</strong> {item.fluency}</p><p><strong>FORMATIVE CHECK:</strong> {item.check}</p><small>{item.spaces}</small><MathAnticsYearPlan lessonIds={item.lessonIds} /><MathGamesFor lessonIds={item.lessonIds} /></section></article>)}</div>
