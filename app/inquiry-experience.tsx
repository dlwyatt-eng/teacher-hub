...
const studentMoves: Record<string, StudentMove[]> = {
  ...
  "signal-case": [
    ...
    { label: "Build the signal pathway", action: "Put the six steps in order from the ruler beginning to fall to your fingers closing.", response: "Six labelled steps with arrows, plus one thing the diagram cannot show" },
  ...
  "balance-case": [
    ...
    { label: "Change one condition", action: "Predict first, then change either heat or water—not both. Compare three settings and record where the model surprises you.", response: "A table comparing three settings, plus one thing the model cannot tell us" },
  ...
  "systems-jigsaw": [
    { label: "Become one-system experts", action: "Read or watch the source your teacher assigns. Name your body system’s job and main parts. Draw arrows to show where messages or materials travel. Explain how another system helps and one thing your drawing cannot show.", response: "A labelled diagram with arrows, notes, and the name of your source" },
    { label: "Prepare a three-minute lesson", action: "Use the six-step checklist below. Prepare a labelled diagram, a small paper circle to move along it, and questions for your classmates. Practise your lesson with a three-minute timer.", response: "A checked and rehearsed three-minute expert lesson" },
    { label: "Teach in mixed groups", action: "Take turns teaching. For each system, write its job, draw where messages or materials travel, and explain how it works with another system.", response: "A four-system comparison with three cross-system connections" },
    { label: "Compare a bear and a human", action: "Use the Bears and Body Systems pages your teacher provides. Choose one body part shown in both animals. Explain how the parts help the bear and human, and use a label or note from the pages to support your comparison.", response: "One comparison, the source that supports it, and one question the pages do not answer" },
  ],
  "life-systems-studio": [
  ...
  "separation-rescue": [
    ...
    { label: "Run, pause, and revise", action: "Carry out the approved plan safely. Halfway through, compare evidence with another team and improve one weak step.", response: "Labels for the materials you separated, a note about one problem, partner feedback, and one improvement" },
    { label: "Evaluate more than recovery", action: "Inspect each material you separated. Check how clean it is, how much you recovered, how much time and material you used, and whether the process was safe.", response: "Two results that show how well your process worked, and one improvement that might make something else harder" },
...
const sciencePrintContent: Record<string, { title: string; directions: string; sections: { heading: string; prompts: string[] }[] }> = {
  ...
  "crash-lab": { title: "Crash Lab: nine trials + explanation", directions: "Keep mass and speed fixed. Change only padding. Record stopping time and largest force for every trial.", sections: [
    ...
    { heading: "Nine-trial data", prompts: ["No padding · stopping time (ms): 42, 44, 43 · largest force (N): 238, 227, 233 · averages: ____ / ____", "Thin foam · stopping time (ms): 78, 82, 80 · largest force (N): 128, 122, 125 · averages: ____ / ____", "Thick foam · stopping time (ms): 145, 150, 147 · largest force (N): 69, 67, 68 · averages: ____ / ____", "Circle what stayed fixed: cart mass · starting speed · barrier · measuring system. Underline the one changed variable: padding condition."] },
    { heading: "Graph + CER", prompts: ["Make two bar graphs, one for average stopping time (ms) and one for average largest force (N). Put padding type along the bottom of each. Use a separate, evenly spaced vertical scale starting at zero for each graph. Add the three trial values and divide by 3 to find each average; round to the nearest whole number.", "Claim: ____________________", "Two results from this simulated model: ____________________", "Reasoning: ____________________", "One thing this model cannot show: ____________________"] },
  ] },
  "movement-forces": { title: "Movement force-frame analysis", directions: "Use the built-in before, during and after kick drawings, or watch your teacher gently roll and stop a ball on a clear table. Sketch three frames below. Label the object receiving a force, what exerts it, and the direction. Finish with one explanation using your observations; athletic ability is not assessed.", sections: [
...
function BalanceLab({ scene }: { scene: number }) {
  ...
  return <section className="evidence-sim feedback-loop-builder"><header><small>BUILD A COORDINATED RESPONSE</small><h3>{loop.length} of {loopParts.length} links built</h3><p>Click the steps in the order the body responds to heat and water loss. Explain how each step leads to the next.</p></header><div className="feedback-loop-route">{loopParts.map((item, index) => <span key={item} className={loop.includes(index) ? "placed" : ""}><b>{index + 1}</b>{loop.includes(index) ? item : "?"}</span>)}</div><div className="feedback-loop-bank">{[3, 0, 4, 1, 2].map(index => <button key={loopParts[index]} disabled={loop.includes(index)} onClick={() => { if (index === loop.length) setLoop(value => [...value, index]); }}>{loopParts[index]}</button>)}</div><footer><b>{loop.length === loopParts.length ? "Feedback loop complete:" : "Explain each arrow:"}</b><span>{loop.length === loopParts.length ? "As the body cools or gets more water, its signals change again. This repeated adjustment is called a feedback loop." : "What changes or travels between these two steps?"}</span></footer></section>;
}
...
function SystemsJigsaw({ scene }: { scene: number }) {
  ...
  const systems = [
    ...
    { name: "Excretory", job: "Remove metabolic wastes and regulate water and dissolved materials in blood.", route: "Kidneys filter blood, which returns to circulation. Urine travels: kidneys → ureters → bladder → urethra.", moves: "blood, filtered materials, returned water/salts, and urine", connection: "Hormonal messages help kidneys adjust how much water is returned to blood." },
  ...
  if (scene === 0) return <section className="evidence-sim system-expert-board"><header><small>YOUR GROUP’S BODY SYSTEM</small><h3>Learn about one body system so you can teach it.</h3><p>Choose the system your teacher assigned. Use these notes and your assigned source to prepare a labelled diagram.</p></header><nav>{systems.map((item, index) => <button key={item.name} className={system === index ? "selected" : ""} onClick={() => setSystem(index)}><b>{index + 1}</b>{item.name}</button>)}</nav><article><small>EXPERT BRIEF</small><h3>{current.name} system</h3><div><b>Main job</b><p>{current.job}</p></div><div><b>Where messages or materials travel</b><p>{current.route}</p></div><div><b>What moves</b><p>{current.moves}</p></div><aside><b>How it works with another system</b><p>{current.connection}</p></aside></article></section>;
  if (scene === 1) {
    const items = [
      { title: "Ask classmates to make a prediction.", example: "For example: What do you think happens to a nerve message after it reaches the brain?" },
      { title: "Draw and label the steps.", example: "Use arrows to show where something travels. For example: eyes → nerves → brain → nerves → hand muscles." },
      { title: "Show what travels along the arrows.", example: "Move a small paper circle along your diagram. Tell classmates what it represents, such as a nerve message." },
      { title: "Explain how two body systems work together.", example: "For example: hormones are messages that help the kidneys adjust how much water stays in the blood." },
      { title: "Explain one thing your model cannot show.", example: "For example: moving a paper circle cannot show how fast a real nerve message travels. This is a limitation of the model." },
      { title: "Ask a question to check understanding.", example: "Prepare the answer too. For example: What carries a message from the brain to hand muscles? Answer: nerves." },
    ];
    return <section className="evidence-sim expert-lesson-builder">
      <header><small>PLAN YOUR THREE-MINUTE BODY-SYSTEM LESSON</small><h3 aria-live="polite">{Object.values(checks).filter(Boolean).length} of {items.length} steps ready</h3><p>Use your group’s body-system diagram to teach classmates how it works. Prepare these six steps on paper. Click each step when it is ready.</p></header>
      <div>{items.map((item, index) => <button key={item.title} type="button" aria-pressed={Boolean(checks[index])} className={checks[index] ? "checked" : ""} onClick={() => setChecks(value => ({ ...value, [index]: !value[index] }))}><b aria-hidden="true">{checks[index] ? "✓" : `${index + 1}.`}</b>{" "}<span><strong>{item.title}</strong><small>{item.example}</small></span></button>)}</div>
      <footer><b>Try it with a partner:</b><span>Give your lesson in three minutes. Then ask your partner to point to the diagram and explain what travels, where it goes, and what happens next.</span></footer>
    </section>;
  }
  if (scene === 2) return <section className="evidence-sim jigsaw-rotation"><header><small>MIXED-GROUP TEACHING</small><h3>Four experts build one connected body map.</h3><p>Make four boxes on paper, one for each system. After each lesson, write its main job, sketch where messages or materials travel, and explain how another system helps.</p></header><div>{systems.map((item, index) => <article key={item.name}><b>{index + 1}</b><strong>{item.name}</strong><span>Explain its job. Trace the arrows. Name what travels. Show how another system helps.</span><small>LISTENERS ASK: What evidence supports that connection?</small></article>)}</div><footer><b>Check your notes:</b><span>For each system, can you explain how it works with another system? Ask the speaker about anything unclear.</span></footer></section>;
  return <section className="evidence-sim multi-system-cases"><header><small>COMPARE BODY PARTS · FNESC/FNSA</small><h3>What can one bear–human comparison teach?</h3><p>Use only the teacher-provided <em>Bears and Body Systems</em> pages. Name the source before making a comparison.</p></header><div className="source-transfer-frame"><article><b>1 · SOURCE EVIDENCE</b><span>Choose one body part shown in both the bear and human pages. Record a label or note from each page that helps you compare them.</span></article><article><b>2 · STRUCTURE → FUNCTION</b><span>Explain how the shape of each body part helps it do its job. Point to a detail on the pages to support your idea.</span></article><article><b>3 · WHAT IS STILL UNKNOWN?</b><span>Write one question these pages do not answer. Keep your conclusions about these examples; they cannot tell us about every animal, person, or First Nation.</span></article></div><footer><b>Credit it exactly:</b><span>FNESC/FNSA · <em>Science First Peoples Teacher Resource Guide (Grades 5–9)</em> · “Bears and Body Systems.” Name this source. Do not use these pages to claim that all First Nations share the same knowledge.</span></footer></section>;
}
...
function SeparationToolkit({ scene }: { scene: number }) {
  ...
  return <section className="mixtures-ui application-board"><header><small>NO SINGLE SECRET SEQUENCE</small><h3>Defend a process for an unfamiliar problem.</h3><p>Choose two cases. Build a sequence, then name a trade-off or limit.</p></header><div>{applicationCases.map((item, index) => <article key={item}><b>{index + 1}</b><strong>{item}</strong><p>Property → method → result → next problem</p><button>Discuss the steps, sketch them in order, and explain why they would work</button></article>)}</div><footer><strong>A strong answer</strong><span>explains why each method follows from evidence and why a tempting alternative would not fully work.</span></footer></section>;
}
...
function LifeSystemsStudio({ scene }: { scene: number }) {
  ...
  if (scene === 0) {
    ...
    return <section className="evidence-sim life-audit">
      <div className="life-statement-list"><small>AFTER THE DIAGRAM LESSON · CHECK ALL FOUR STATEMENTS</small>{statements.map((item, index) => <button key={item.text} className={selected === index ? "selected" : ""} onClick={() => setSelected(index)}><span>{index + 1}</span>{item.text}</button>)}</div>
...
function BodyCaseConference({ scene }: { scene: number }) {
  ...
  const current = cases[caseIndex];
  if (scene === 0) return <section className="evidence-sim case-triage"><header><small>READ THE CLUES · FICTIONAL CASE</small><h3>{current.title}</h3><p>Select a case, then choose systems only when a clue requires them.</p></header><nav>{cases.map((item, index) => <button key={item.title} className={caseIndex === index ? "selected" : ""} onClick={() => { setCaseIndex(index); setSystems({}); }}>{item.title}</button>)}</nav><div className="case-evidence-list">{current.evidence.map((item, index) => <article key={item}><b>{index + 1}</b>{item}</article>)}</div><div className="case-system-bank">{allSystems.map(item => <button key={item} className={systems[item] ? (current.systems.includes(item) ? "supported" : "unsupported") : ""} onClick={() => setSystems(value => ({ ...value, [item]: !value[item] }))}>{item}</button>)}</div><footer><b>What this case cannot tell us:</b><span>{current.caution}</span></footer></section>;
  if (scene === 1) { const items = ["I can point to a case clue that explains why I included each body system.", "I have labelled both ends of every arrow so a reader knows what is connected.", "Each arrow says what happens, such as “sends a message” or “filters blood”.", "At least one arrow shows how two body systems work together.", "I added a note explaining one thing the case or my model cannot tell us."]; return <section className="evidence-sim causal-model-check"><header><small>CHECK YOUR BODY-SYSTEM DIAGRAM</small><h3>{Object.values(buildChecks).filter(Boolean).length} of {items.length} checks complete</h3><p>Start with a labelled diagram on paper. Use arrows to explain what happens and why. Click each check when your diagram shows it; then review it with your teacher.</p></header><div>{items.map((item, index) => <button key={item} aria-pressed={Boolean(buildChecks[index])} className={buildChecks[index] ? "checked" : ""} onClick={() => setBuildChecks(value => ({ ...value, [index]: !value[index] }))}><b aria-hidden="true">{buildChecks[index] ? "✓" : `${index + 1}.`}</b>{" "}<span>{item}</span></button>)}</div><footer><b>Example connection:</b><span>Eyes notice the starting light → nerves carry a message to the brain → the brain sends messages through nerves → hand muscles move.</span></footer></section>; }
  if (scene === 2) { const items = ["The claim answers the case question", "At least two systems have clear jobs", "A listener can follow every arrow", "Evidence—not just vocabulary—supports the explanation", "The team names what its model cannot conclude"]; return <section className="evidence-sim peer-case-panel"><header><small>TWO-MINUTE CASE CONFERENCE</small><h3>Listeners test the explanation.</h3><p>Click a check only when the speakers have explained it. Ask them how one step leads to the next, or which case clue supports their idea.</p></header><div>{items.map((item, index) => <button key={item} aria-pressed={Boolean(peerChecks[index])} className={peerChecks[index] ? "checked" : ""} onClick={() => setPeerChecks(value => ({ ...value, [index]: !value[index] }))}><b aria-hidden="true">{peerChecks[index] ? "✓" : `${index + 1}.`}</b>{" "}<span>{item}</span></button>)}</div><aside><b>Try asking</b><p>“What evidence shows that the _____ system is needed between _____ and _____?”</p></aside></section>; }
  return <section className="evidence-sim case-revision"><header><small>VISIBLE REVISION</small><h3>Improve the explanation—not just the appearance.</h3><p>Choose the change that would help a learner most.</p></header><div>{["Repair a weak or missing arrow", "Add evidence beside a claim", "Remove a system the case does not require", "Clarify what moves between systems", "Add a note about one thing the model cannot show"].map((item, index) => <article key={item}><b>{index + 1}</b><strong>{item}</strong><span>Before: _____ → After: _____ → Why this is stronger: _____</span></article>)}</div><footer><b>Keep your diagram in class.</b><span>Keep it with your science notes so you can use it in a later project. No separate SpacesEDU post is needed.</span></footer></section>;
}
...
function DeliveryPodLab({ scene }: { scene: number }) {
  ...
  const portfolioItems = ["Team design photo or Minecraft model", "Results table with units", "One design feature explained using forces, such as padding that makes the stop last longer", "Evidence that led to revision", "What the tests cannot prove", "One next test or design change"];
...
function ForcePatternsLab({ scene }: { scene: number }) {
  ...
  if (scene === 1) {
    ...
    return <section className="evidence-sim station-rotation-board"><header><small>FOUR STATIONS · ONE EVIDENCE ROUTINE</small><h3>Predict → change one condition → repeat → describe the pattern</h3><p>Mark a station complete only when the group has measured or clearly observed a change and recorded a limitation.</p></header><div>{stationNames.map((name, index) => <button key={name} className={stations[index] ? "checked" : ""} onClick={() => setStations(value => ({ ...value, [index]: !value[index] }))}><b>{stations[index] ? "✓" : index + 1}</b><span><strong>{name}</strong><small>Draw the forces. Record results. Describe a pattern and one thing the model leaves out.</small></span></button>)}</div><footer><strong>{Object.values(stations).filter(Boolean).length} / 4 stations recorded</strong><span>One result is not a pattern. Repeat before moving on.</span></footer></section>;
  }
  if (scene === 2) {
...
function MovementForcesLab({ scene }: { scene: number }) {
  ...
  if (scene === 1) {
    ...
    return <section className="evidence-sim movement-force-map"><header><small>OBJECT–INTERACTION MAP</small><h3>Which object receives each force?</h3><p>Action–reaction forces belong on different objects. Balanced arrows do not mean “no forces.”</p></header><div className="movement-map-stage"><span>PERSON</span><i>GROUND</i>{forceLabels.map((item, index) => <button key={item} className={arrows[index] ? "selected" : ""} onClick={() => setArrows(value => ({ ...value, [index]: !value[index] }))}><b>{arrows[index] ? "✓" : "+"}</b>{item}</button>)}</div><footer><strong>{Object.values(arrows).filter(Boolean).length} force labels selected</strong><span>On paper, draw each selected force as an arrow starting on the object receiving it. Label what exerts the force and its direction.</span></footer></section>;
  }
  if (scene === 2) {
    const routes = ["Drawing route · annotate the built-in three-frame kick sequence", "Object test · roll, stop, and change surfaces", "Seated movement · safe push and stop", "Standing movement · low-intensity balance or landing"];
    return <section className="evidence-sim movement-choice-lab"><header><small>FOUR EQUALLY VALID ROUTES</small><h3>Choose evidence—not performance.</h3><p>Start with the built-in kick drawings from part 1, or ask your teacher to gently roll and stop a ball on a clear table. Sketch before, during and after contact. Nobody needs to jump, race, or be filmed.</p></header><div>{routes.map((item, index) => <button key={item} className={route === index ? "selected" : ""} onClick={() => setRoute(index)}><b>{index + 1}</b><span>{item}</span></button>)}</div><aside><b>Your one changed condition</b><p>Change _____; observe or measure _____; keep _____ the same; stop if _____.</p></aside></section>;
  }
  const claims = [
