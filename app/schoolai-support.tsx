...
import { schoolAILessonCoaching } from "./schoolai-lesson-coaching";
...
export function SchoolAISupport({ lessonId, title, question, product }: { lessonId?: string; title: string; question: string; product: string }) {
  ...
  return <section className="schoolai-support" aria-label="Optional SchoolAI support">
    ...
    <details><summary>Choose a coach and prepare SchoolAI</summary>
      ...
      <p><strong>Finish:</strong> {lessonCoach?.finish ?? coach[2]} Keep the lesson's usual hand-in or SpacesEDU route.</p>
      <p><strong>Without AI:</strong> {lessonCoach?.withoutAi ?? `Use the same coaching move with a partner or teacher: ${coach[1]}`}</p>
      ...
      <div className="schoolai-support-controls"><button type="button" onClick={async () => { try { await navigator.clipboard.writeText(prompt + specificInstructions); setMessage("Instructions copied. Paste into your SchoolAI Space and preview before launch."); } catch { setMessage("Select and copy the instructions below."); } }}>Copy coach instructions</button><a href="https://app.schoolai.com/" target="_blank" rel="noreferrer">Open SchoolAI for setup ↗</a></div>
      <p role="status">{message}</p>
      <label htmlFor={`${id}-prompt`}>Instructions to copy<textarea id={`${id}-prompt`} readOnly value={prompt + specificInstructions} rows={8} /></label>
