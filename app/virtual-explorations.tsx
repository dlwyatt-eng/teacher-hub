...
export function ExplorationAnnualMap({ onExplore }: { onExplore?: (destination: ExplorationDestination, student: boolean) => void }) {
  return <details id="virtual-explorations" className="exploration-annual"><summary>Virtual explorations · flexible annual map</summary>
    ...
    <div className="exploration-annual__table"><table><caption>World, space, body and animal viewpoints · reviewed {register.reviewDate}</caption><thead><tr><th scope="col">Teaching window</th><th scope="col">Visit</th><th scope="col">Lesson and subject</th><th scope="col">Open</th></tr></thead><tbody>{register.placements.map(p => <tr key={p.tourId}><td>{p.window}</td><td>{register.trips.find(t => t.id === p.tourId)?.title}</td><td>{p.subject}<br />{p.lessonTitle}</td><td><a style={{ display: "inline-block", minWidth: "8rem", minHeight: 44, padding: ".4rem 0" }} href={explorationLessonUrl(p)} onClick={event => { if (onExplore && event.button === 0 && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.altKey) { event.preventDefault(); onExplore(p, false); } }}>Teacher lesson</a><br /><a style={{ display: "inline-block", minWidth: "8rem", minHeight: 44, padding: ".4rem 0" }} href={explorationLessonUrl(p, true)} onClick={event => { if (onExplore && event.button === 0 && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.altKey) { event.preventDefault(); onExplore(p, true); } }}>Project lesson</a></td></tr>)}</tbody></table></div>
  </details>;
}
