...
The new content/SSR checks cover real lesson and scene targets, canonical URLs, teacher-answer separation, no eager media, one mounted space alternative and printable backups outside closed disclosures. Release checks passed: Teacher 120/120, Learn 13/13, Equity 22/22; TypeScript and static artifact validation passed for all three repositories. Local supported preview was healthy, but its browser route was blocked by the browsing environment; this does not constitute visual verification. Public checks after deployment: Teacher architecture lesson, Learn kidney backup and Equity lesson entry rendered successfully at desktop size. Student/family DOM excluded teacher notes. Loading the Learn video created one iframe; choosing the backup removed it. The annual map’s direct lesson URL resolved correctly; its in-app route now uses the existing navigation callback, and keyboard activation passed. Annual-map links have a continuous 44px click target so wrapped text cannot leave a gap in the target. Sustained embedded playback, phone layout, print output and school-network compatibility remain unconfirmed. A successful software test does not establish media playback or classroom suitability.
